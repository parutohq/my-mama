...
export default function SignInPage() {
  ...
  return <main id="main-content" className="auth-page" tabIndex={-1}><section className="auth-card"><p className="eyebrow">MAMA</p><h1>Your personal care space</h1><p>Sign in to keep your records private and available across devices.</p><form onSubmit={submit}><label>Email<input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" /></label>{mode !== 'recovery' && <label>Password<input required minLength={8} type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete={mode === 'sign-up' ? 'new-password' : 'current-password'} /></label>}<button disabled={busy} type="submit">{busy ? 'Please wait…' : mode === 'sign-up' ? 'Create account' : mode === 'recovery' ? 'Send recovery email' : 'Sign in'}</button></form>{message && <output>{message}</output>}<p><button type="button" className="text-button" onClick={() => setMode(mode === 'sign-in' ? 'sign-up' : 'sign-in')}>{mode === 'sign-in' ? 'Create an account' : 'I already have an account'}</button></p><p><button type="button" className="text-button" onClick={() => setMode('recovery')}>Forgot your password?</button></p><p className="auth-preview-link"><a href="/design-preview">Explore the Home design preview</a></p></section></main>;
}
