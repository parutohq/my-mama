  ...
  async function submit(event: { preventDefault(): void }) {
    ...
    try {
      if (mode === 'recovery') {
        ...
        const { error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent('/')}` } });
