...
export function AuthForm({ initialMode = 'sign-in', initialMessage }: Props) {
  ...
  async function submit(event: { preventDefault(): void }) {
    ...
    setBusy(true); setMessage(''); setCanResend(false);
    ...
    if (mode === 'recovery') {
      ...
      setCanResend(!error); setMessage(error ? 'We could not create your account. Please check the details and try again.' : 'Check your email to confirm your private MAMA account. If you already have an account, sign in or request a password reset instead.');
    } else {
  ...
  async function resendConfirmation() {
    ...
    const { error } = await createClient().auth.resend({ type: 'signup', email, options: { emailRedirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent('/sign-in?verified=1')}` } });
    setMessage(error ? 'We could not resend that confirmation email. Please check the address and try again.' : 'A new confirmation email is on its way.');
    setBusy(false);
  }
  ...
  return <main id="main-content" className="auth-page"><section className="auth-card">
    ...
    {message && <output aria-live="polite">{message}</output>}{canResend && <button type="button" className="text-button auth-resend" disabled={busy} onClick={resendConfirmation}>Resend confirmation email</button>}
